...
bool AlsaInput::open()
{
    ...
    {
        ret = snd_pcm_open(&pcm_handle, hw_device.c_str(), SND_PCM_STREAM_CAPTURE, SND_PCM_NONBLOCK);
...
unsigned int AlsaInput::getFileDescriptors()
{
    return nfds;
}

void AlsaInput::getPollFd(struct pollfd *_pfds)
{
    if (static_cast<unsigned int>(snd_pcm_poll_descriptors(pcm_handle, pfds, nfds)) != nfds)
    {
        printf("cannot obtain capture file descriptors\n");
    }
...
int AlsaInput::checkPolling(struct pollfd *_pfds)
{
   unsigned short revents=0;
   int err;
   if ((err = snd_pcm_poll_descriptors_revents(pcm_handle, _pfds, nfds, &revents)) < 0)
   {
      fprintf (stderr, "error in capture_ready: %s\n", snd_strerror (err));
